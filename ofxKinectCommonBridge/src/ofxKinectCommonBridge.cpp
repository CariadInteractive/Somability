...
void ofxKinectCommonBridge::threadedFunction(){
	...
	//	printf("loop\n");
		...
		if(bVideoIsInfrared)
		{
			bool bb = KinectIsColorFrameReady(hKinect);
			if(  bb && SUCCEEDED( KinectGetColorFrame(hKinect, colorFormat.cbBufferSize, irPixelByteArray, &timestamp) ) )
			{
		...
		{
			int res = KinectGetColorFrame(hKinect, colorFormat.cbBufferSize, videoPixelsBack.getPixels(), &timestamp);
			if( SUCCEEDED( res ) )
			{
				bNeedsUpdateVideo = true;

			} else { 
				//printf("couldnt get frame %d\n", res);
			//	_com_error err(hr);
